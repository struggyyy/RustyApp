...
const PanelContainer = styled.View<{ isExpanded: boolean }>`
  ...
  padding: 20px;
  margin-bottom: 12px;
  min-height: ${(props: { isExpanded: boolean }) =>
    props.isExpanded ? "auto" : "80px"};
`;
...
const FilterChip = styled.TouchableOpacity<{
  ...
  background-color: ${(props: { isSelected: boolean; chipColor?: string }) =>
    props.isSelected
      ? props.chipColor || theme.colors.primary
      : theme.colors.white};
  border-radius: 16px;
  padding: 10px 12px;
  align-items: center;
  border-width: 1px;
  border-color: ${(props: { isSelected: boolean; chipColor?: string }) =>
    props.isSelected
      ? props.chipColor || theme.colors.primary
...
const MapButtonView = styled.View({
  width: 56,
  height: 56,
  borderRadius: 28,
...
const FilterPanel: React.FC<FilterPanelProps> = ({
  ...
  const [selectedDistanceIndex, setSelectedDistanceIndex] = React.useState(
    (maxDistance || 5) - 1
  );
      ...
      <CollapsedFilterContent>
        <CollapsedFilterTop>
          ...
          >
            ...
            <View style={{ marginTop: 4, pointerEvents: "auto" }}>
              <MaterialIcons
