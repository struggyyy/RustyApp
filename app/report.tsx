...
const SubmitButton = styled.TouchableOpacity<{ isDisabled: boolean }>`
  background-color: ${(props: { isDisabled: boolean }) => (props.isDisabled ? colors.text.tertiary : colors.componentBackground)};
  width: 90%;
  padding: 16px;
  border-radius: 16px;
  align-items: center;
...
export default function ReportScreen() {
  ...
  const handleTakePicture = () => console.log('Take Picture pressed');
  const handleSelectImage = () => console.log('Select Image pressed');

  const handleMarkLocation = () => {
    if (location && mapRef.current && mapRegion) {
  ...
  const renderMapContent = () => {
    if (isLocationLoading) {
      return <ActivityIndicator size="large" color={colors.text.secondary} />;
    }
    if (locationErrorMsg) {
      ...
      >
        ...
        <MainCard isKeyboardVisible={isKeyboardVisible}>
          ...
          {!isKeyboardVisible && (
            <MapContainer>
              {renderMapContent()}
              <MapOverlayButton onPress={handleMarkLocation}>
        ...
        <BottomContent>
          ...
          {!isKeyboardVisible && (
            <SubmitButton onPress={handleSubmit} isDisabled={isSubmitting}>
              {isSubmitting ? (
